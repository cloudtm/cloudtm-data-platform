...
public class RequestCorrelator {
    ...
    protected void handleRequest(Message req, Header hdr) {
       ...
       messageRequest.sendReply(retval, threwException);
    }
   ...
   private class MessageRequestImpl implements MessageRequest {
      private final Message request;
      private final Header header;

      private MessageRequestImpl(Message request, Header header) {
      ...
      public final void sendReply(Object reply, boolean exceptionThrown) {
         if(!header.rsp_expected || reply == RequestHandler.DO_NOT_REPLY) // asynchronous call, we don't need to send a response; terminate call here
            return;
         ...
         try {  // retval could be an exception, or a real value
            ...
            try {  // this call should succeed (all exceptions are serializable)
               ...
               if(log.isErrorEnabled()) log.error("failed sending rsp: return value (" + reply + ") is not serializable");
               return;
            }
         }

         Message response = request.makeReply();
         prepareResponse(response);
         response.setFlag(Message.OOB);
         response.setFlag(Message.DONT_BUNDLE);
         if(request.isFlagSet(Message.NO_FC))
            response.setFlag(Message.NO_FC);
         if(request.isFlagSet(Message.NO_RELIABILITY))
            response.setFlag(Message.NO_RELIABILITY);
         if(request.isFlagSet(Message.NO_TOTAL_ORDER))
            response.setFlag(Message.NO_TOTAL_ORDER);

         if(rsp_buf instanceof Buffer)
            response.setBuffer((Buffer)rsp_buf);
         else if (rsp_buf instanceof byte[])
            response.setBuffer((byte[])rsp_buf);

         Header responseHeader = new Header(exceptionThrown ? Header.EXC_RSP : Header.RSP, header.id, false,
                                            RequestCorrelator.this.id);
         response.putHeader(RequestCorrelator.this.id, responseHeader);
         if(log.isTraceEnabled())
            log.trace(new StringBuilder("sending rsp for ").append(responseHeader.id).append(" to ")
                            .append(response.getDest()));

         transport.down(new Event(Event.MSG, response));
      }
   }
