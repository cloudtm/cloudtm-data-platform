 ...
 * 
 * Pedro Ruivo changes:
 *    - using MurmurHash3 ISPN implementation instead of MD5
 *    - the bloom filter accepts any kind of objects
 *
...
public class BloomFilter implements Serializable {
   ...
   public static int[] createHashes(byte[] data, int hashes) {
      return createMurmurHashes3(data, hashes);
   }

   public static int[] createMurmurHashes3(Object object, int hashes) {
      ...
      if (object instanceof byte[]) {
         ...
         key = ((String) object).getBytes(charset);
      }else {
         key = new byte[4];
         int hasCode = object.hashCode();
         for (int i = 0; i < 4; ++i) {
            key[i] = (byte) (hasCode >> (8 * i));
         }
      }
      ...
      int salt = 31;
      while (k < hashes) {
         result[k++] = key == null ? MurmurHash3.MurmurHash3_x64_32(lKey, salt) :
               MurmurHash3.MurmurHash3_x64_32(key, salt);
         salt *= 7;
      }
      return result;
   }
   ...
   public void add(Object element) {
      int[] hashes = createMurmurHashes3(element, k);
      for (int hash : hashes) {
